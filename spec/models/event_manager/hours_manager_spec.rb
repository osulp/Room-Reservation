...
describe EventManager::HoursManager do
  ...
  describe ".events_between" do
    ...
    context "when hours are 1:00 PM to 10:00 PM" do
      ...
      it "should lock up the schedule from 21:50:00 to midnight" do
        expect(subject[1].start_time).to eq Time.current.midnight+21.hours+50.minutes
    ...
    context "when hours are 12:15 AM to 4:00 PM" do
      ...
      it "should only lock the schedule from 3:50 to midnight" do
        expect(subject.first.start_time).to eq Time.current.midnight+15.hours+50.minutes
        expect(subject.first.end_time).to eq Time.current.tomorrow.midnight
      end
    end
