  ...
  describe "#time_info" do
    ...
    context "when given a date outside the term" do
      it "should return an empty hash" do
        Hour.time_info(Date.today-30.days).should == {}
      end
    end
    context "when given a monday inside the term" do
      it "should return the start and end time for that day" do
        expect(Hour.time_info(Date.today)).to eq({Date.today => {'open' => "10:00 am", 'close' => "10:00 pm"}})
      end
      it "should return the same value for Monday->Thursday" do
        3.times do |n|
          expect(Hour.time_info(Date.today+n.days)).to eq({Date.today+n.days => {'open' => "10:00 am", 'close' => "10:00 pm"}})
        end
      end
      it "should be able to return the time on sundays" do
        expect(Hour.time_info(Date.today+6.days)).to eq({Date.today+6.days => {'open' => '5:00 am', 'close' => '6:00 pm'}})
      end
    end
  end
end