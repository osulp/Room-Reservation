class EventManager::ReservationManager < EventManager::EventManager
  ...
  def to_event(reservation)
    end_time = reservation.truncated_at if reservation.truncated_at && reservation.truncated_at < reservation.end_time && reservation.truncated_at >= reservation.start_time
    end_time ||= reservation.end_time
    Events::ReservationEvent.new(reservation.start_time-reservation_padding, end_time+reservation_padding, priority, reservation, reservation.room_id)
  end

  # @TODO: Move this into configuration
  def reservation_padding
    @reservation_padding ||= Setting.reservation_padding.to_i.minutes
  end
